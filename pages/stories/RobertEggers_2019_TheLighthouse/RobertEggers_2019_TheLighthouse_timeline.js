...
function showInfoCard(d, event, updateInPlace = false) {
    ...
    const previousId = hasPrevious ? SCENES[currentIndex - 1].id : null;
    const nextId = hasNext ? SCENES[currentIndex + 1].id : null;

    // Build location/time display
    let locationTimeHtml = '';
    if (scene.location || scene.time) {
        locationTimeHtml = '<div class="info-card-location">';
        if (scene.location) {
            locationTimeHtml += `<div class="location-text">📍 ${scene.location}</div>`;
        }
        if (scene.time) {
            locationTimeHtml += `<div class="time-text">🕐 ${scene.time}</div>`;
        }
    ...
    const html = `
        ...
        <div class="info-card-body">
            ...
            ${scene.imagery ? `
            <p class="info-card-imagery"><strong>🎬 Visual Imagery:</strong> ${scene.imagery}</p>
            ` : ''}
            ${scene.themes && scene.themes.length > 0 ? `
            <p class="info-card-themes"><strong>🎭 Themes:</strong> ${scene.themes.join(', ')}</p>
            ` : ''}
            ${scene.mythologyRef ? `
            <p class="info-card-mythology"><strong>📚 Mythology:</strong> ${scene.mythologyRef}</p>
            ` : ''}
            ${scene.symbols && scene.symbols.length > 0 ? `
            <p class="info-card-symbols"><strong>🔑 Key Symbols:</strong> ${scene.symbols.join(', ')}</p>
            ` : ''}
        </div>
    ...
    // Update locked node state for keyboard navigation
    state.lockedNode = d;
...
function jumpToSceneAndUpdate(sceneId) {
    const targetNode = state.root.descendants().find(d => d.data.id === sceneId);
    if (targetNode) {
        ...
        // Scroll to top of the card
        const infoCardNode = infoCard.node();
        if (infoCardNode) {
            infoCardNode.scrollTop = 0;
        }
    }
}
...
function buildCharacterSection(scene) {
    ...
    let html = '<div class="info-card-characters"><strong>👤 Characters:</strong><div class="characters-grid">';

    if (scene.characters.young) {
        html += `<div class="character-state"><strong>Young:</strong> ${scene.characters.young}</div>`;
    }
    if (scene.characters.old) {
        html += `<div class="character-state"><strong>Old:</strong> ${scene.characters.old}</div>`;
    }
...
function buildDialogueSection(scene) {
    ...
    scene.quotes.forEach(dialogue => {
        ...
        if (speaker && quote) {
            html += `<div class="quote-item"><strong>${speaker}:</strong> "${quote}"</div>`;
        }
    });
...
function getTensionColor(tension) {
    ...
    if (tension <= 5) return '#ffc107'; // Yellow - medium tension
    if (tension <= 7) return '#ff9800'; // Orange - high tension
    return '#c62828'; // Dark red - extreme tension
}
    ...
    let html = `
        <div class="screenplay-viewer-header">
            <div class="screenplay-viewer-title">
                ...
                <div>
                    <div class="screenplay-title">The Lighthouse Screenplay</div>
...
function initSearch() {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (infoCard && infoCard.classed('visible')) {
            if (state.lockedNode) {
                const currentIndex = SCENES.findIndex(s => s.id === state.lockedNode.data.id);
                if (currentIndex !== -1) {
                    if (e.key === 'ArrowLeft' && currentIndex > 0) {
                        // Previous scene
                        e.preventDefault();
                        window.navigateToScene(SCENES[currentIndex - 1].id);
